...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(Input.GetKeyDown(jumpKey) && !isGrounded && jumpTimer < 0)
        {
            if(canDoubleJump)
            {
                rb.AddForce(0f, jumpPower * 2, 0f, ForceMode.Impulse);
                canDoubleJump = false;
            }
        }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.CompareTag("Moving")) {
            Debug.Log("Moving Plat");
            gameObject.transform.parent.gameObject.transform.SetParent(collision.gameObject.transform);
